...
class UserController {
  ...
  async index(req, res) { // sempre usar o try/catch quando usar async/await
    try {
      const showUsers = await _User2.default.findAll({ attributes: ['id', 'nome', 'email'] }); // setando apenas o id, nome e email para serem enviados na requisição, melhorando um pouco a questão da falha de segurança em ter uma listagem de usuários sendo permitida apenas de o usuário logar.
  ...
  async show(req, res) {
    try {
      const showUser = await _User2.default.findByPk(req.params.id);
...
exports. default = new UserController(); // Exportando a classe já instanciada pois o que queremos aqui nesse caso é o objeto da classe, e não a classe em si
